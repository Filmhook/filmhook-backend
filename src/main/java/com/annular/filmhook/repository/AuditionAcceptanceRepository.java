...
public interface AuditionAcceptanceRepository extends JpaRepository<AuditionAcceptanceDetails, Integer> {
	...
	@Query("SELECT a FROM AuditionAcceptanceDetails a WHERE a.auditionAcceptanceUser = :userId")
	List<AuditionAcceptanceDetails> findByUserId(@Param("userId") Integer userId);
