...
public class AuthenticationServiceImpl implements AuthenticationService {
	...
	public ResponseEntity<?> register(UserWebModel userWebModel, String request) {
		...
		try {
			...
			// userRepository.findByEmailAndUserType(userWebModel.getEmail(),
			...
			if (userData.isEmpty()) {
				...
				user.setDob(Utility.isNullOrBlankWithTrim(userWebModel.getDob()) ? ""
						: CalendarUtil.convertDateFormat(CalendarUtil.UI_DATE_FORMAT, CalendarUtil.MYSQL_DATE_FORMAT,
								userWebModel.getDob()));
				...
				// if (!sendVerificationRes) return
				// ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(new Response(-1,
				// "Mail not sent", "error"));
...
//				}
	...
	public ResponseEntity<?> forgotPassword(UserWebModel userWebModel, HttpServletRequest request) {
		...
		return ResponseEntity.ok().body(new Response(1,
...
//			Optional<User> data = userRepository.findByResetPassword(userDetails.userInfo().getId());
	...
	public ResponseEntity<?> addSecondaryMobileNo(UserWebModel userWebModel) {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("Error adding secondary mobile number: " + e.getMessage());
		}
	}
	...
	public ResponseEntity<?> verifyMobileOtp(UserWebModel userWebModel) {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("Error verifying OTP and updating phone number: " + e.getMessage());
		}
	}
