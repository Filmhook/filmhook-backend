...
//              promote.setCountry(promoteWebModel.getCountry());
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new Response(-1, "Promote with ID " + promoteId + " not found", null));
            }
            logger.info("updatePromote method end");
        } catch (Exception e) {
            logger.error("Error updating promote: {}", e.getMessage());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new Response(-1, "Failed to update promote", e.getMessage()));
        }
    ...
    public ResponseEntity<?> deletePromote(PromoteWebModel promoteWebModel) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new Response(-1, "Delete promote failed", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getAllPromote(PromoteWebModel promoteWebModel) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new Response(-1, "Failed to fetch promotes", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getByPromoteId(PromoteWebModel promoteWebModel) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new Response(-1, "Failed to fetch promote", e.getMessage()));
        }
    }
