...
public class PostServiceImpl implements PostService {
		...
		try {
			...
			if (userFromDB != null) {
				...
				Posts posts = Posts.builder().postId(UUID.randomUUID().toString())
						.description(postWebModel.getDescription()).user(userFromDB).status(true)
						.privateOrPublic(postWebModel.getPrivateOrPublic()).promoteFlag(false)
						.createdBy(postWebModel.getUserId()).locationName(postWebModel.getLocationName())
		...
		try {
			if (!Utility.isNullOrEmptyList(postList)) {
				postList.stream().filter(Objects::nonNull).forEach(post -> {
					...
					List<FilmProfessionPermanentDetail> professionPermanentDataList = filmProfessionPermanentDetailRepository
							.findByUserId(post.getUser().getUserId());
					...
					List<FollowersRequest> followersList = friendRequestRepository
							.findByFollowersRequestReceiverIdAndFollowersRequestIsActive(post.getUser().getUserId(),
									true);
		...
		try {
			...
			if (existingPost != null) {
				...
				if (existingLike != null) {
					...
					likeRowToSaveOrUpdate = Likes.builder().likedBy(likeWebModel.getUserId())
							.postId(existingPost.getId()).liveDate(null).status(true)
				...
				existingPost.getLikesCollection().add(savedLikes); // Adding saved/updated likes into
																	// postLikesCollection
				// Integer currentPostTotalLikes = existingPost.getLikesCollection() != null ?
				// (int)
				// existingPost.getLikesCollection().stream().filter(Likes::getStatus).count() :
				// 0;
	...
	private LikeWebModel transformLikeData(Likes likes, Integer totalCount) {
		return LikeWebModel.builder().userId(likes.getLikedBy()).postId(likes.getPostId()).likeId(likes.getLikeId())
				.totalLikesCount(totalCount).status(likes.getStatus()).createdBy(likes.getCreatedBy())
				.createdOn(likes.getCreatedOn()).updatedBy(likes.getUpdatedBy()).updatedOn(likes.getUpdatedOn())
				.build();
	}
	...
	public CommentWebModel addComment(CommentWebModel commentWebModel) {
		try {
			...
			if (post != null) {
				Comment comment = Comment.builder().content(commentWebModel.getContent())
						.commentedBy(commentWebModel.getUserId()).postId(post.getId()).status(true).build();
				...
				Long currentTotalCommentCount = post.getCommentCollection() != null
						? post.getCommentCollection().stream().filter(Comment::getStatus).count()
						: 0;
	...
	public List<CommentWebModel> getComment(CommentWebModel commentWebModel) {
		try {
			Posts post = postsRepository.findById(commentWebModel.getPostId()).orElse(null);
			if (post != null) {
				List<Comment> commentData = post.getCommentCollection() != null
						? (List<Comment>) post.getCommentCollection()
						: new ArrayList<>();
				// Filter comments where status is true
				List<Comment> filteredCommentData = commentData.stream().filter(Comment::getStatus)
		...
		try {
			...
			if (post != null) {
				...
				if (comment != null) {
					...
					post.getCommentCollection().removeIf(val -> val.getCommentId().equals(comment.getCommentId())); // removing
																													// saved/updated
																													// likes
																													// into
	...
	public ShareWebModel addShare(ShareWebModel shareWebModel) {
		try {
			...
			if (existingPost != null) {
				Share share = Share.builder().sharedBy(shareWebModel.getUserId()).postId(existingPost.getId())
						.status(true).createdBy(shareWebModel.getUserId()).createdOn(new Date()).build();
				Share savedShare = shareRepository.saveAndFlush(share); // Save the updated like
				existingPost.getShareCollection().add(savedShare); // Adding saved/updated likes into
																	// postShareCollection
				Integer currentTotalShareCount = existingPost.getShareCollection() != null
						? (int) existingPost.getShareCollection().stream().filter(Share::getStatus).count()
						: 0;
	...
	public LinkWebModel addLink(LinkWebModel linkWebModel) {
		Link link = Link.builder().links(linkWebModel.getLinks()).status(linkWebModel.getStatus())
				.createdBy(linkWebModel.getCreatedBy()).createdOn(linkWebModel.getCreatedOn())
				.updatedBy(linkWebModel.getUpdatedBy()).updatedOn(linkWebModel.getUpdatedOn())
				.userId(linkWebModel.getUserId()).build();
