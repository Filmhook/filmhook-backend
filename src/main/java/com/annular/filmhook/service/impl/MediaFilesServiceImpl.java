...
public class MediaFilesServiceImpl implements MediaFilesService {
	...
	public List<FileOutputWebModel> getMediaFilesByUserAndCategory() {
		...
		try {
			...
			// mediaFilesRepository.getMediaFilesByUserIdAndCategory();
	...
	@Override
	public FileOutputWebModel saveMediaFiles(StoriesWebModel fileInput, User user) {
		FileOutputWebModel fileOutputWebModel = null;
		try {
			// 1. Save first in MySQL
			Story storiesData = this.prepareMediaFileData(fileInput, user);
			storyRepository.save(storiesData);
			logger.info("File id saved in mysql :- {}", storiesData.getFileId());

			// 2. Upload into S3
			File file = File.createTempFile(storiesData.getFileId(), null);
			FileUtil.convertMultiPartFileToFile(fileInput.getFileInputWebModel().getFile(), file);
			String response = fileUtil.uploadFile(file, storiesData.getFilePath());
			logger.info("File Upload in S3 response :- {}", response);
			if (response != null && response.equalsIgnoreCase("File Uploaded")) {
				file.delete(); // deleting temp file
				fileOutputWebModel = this.transformData(storiesData); // Reading the saved file details
			}
		} catch (Exception e) {
			logger.error("Error at saveGalleryFiles()...", e);
			e.printStackTrace();
	...
	private Story prepareMediaFileData(StoriesWebModel fileInput, User user) {
		MultipartFile file = (fileInput.getFileInputWebModel().getFile());

		Story stories = new Story();
		stories.setUser(user);
		stories.setCategory(fileInput.getFileInputWebModel().getCategory());
		stories.setCategoryRefId(fileInput.getFileInputWebModel().getCategoryRefId());
		stories.setDescription(fileInput.getDescription());
		stories.setFileId(UUID.randomUUID().toString());
		stories.setFileName(file.getOriginalFilename());
		stories.setFilePath(
				FileUtil.generateFilePath(stories.getUser(), fileInput.getFileInputWebModel().getCategory(), stories.getFileId()));
		stories.setFileType(file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")));
		stories.setFileSize(file.getSize());
		stories.setStatus(true);
		stories.setCreatedBy(user.getUserId());
		stories.setCreatedOn(new Date());
		// mediaFiles.setUpdatedBy(fileInput.getUserId());
		// mediaFiles.setUpdatedOn(new Date());
		logger.info("storyFile details to save in MySQL :- {}",stories);
		...
		try {
			MultiMediaFiles multiMediaFiles = new MultiMediaFiles();
			multiMediaFiles.setFileName(stories.getFileName());
			multiMediaFiles.setFileOriginalName(fileInput.getFileInputWebModel().getFile().getOriginalFilename());
			multiMediaFiles.setFileDomainId(FilmHookConstants.STORIES);
			System.out.println(stories.getId());
			multiMediaFiles.setFileDomainReferenceId(stories.getId());
			multiMediaFiles.setFileIsActive(true);
			multiMediaFiles.setFileCreatedBy(user.getUserId());
			multiMediaFiles.setFileSize(stories.getFileSize());
			multiMediaFiles.setFileType(stories.getFileType());
			multiMediaFiles = multiMediaFilesRepository.save(multiMediaFiles);
			logger.info("MultiMediaFiles entity saved in the database with ID: {}",
					multiMediaFiles.getMultiMediaFileId());
		} catch (Exception e) {
	...
	private FileOutputWebModel transformData(Story storiesData) {
		FileOutputWebModel fileOutputWebModel = null;
		try {
			fileOutputWebModel = new FileOutputWebModel();

			fileOutputWebModel.setId(storiesData.getId());

			fileOutputWebModel.setUserId(storiesData.getUser().getUserId());
			fileOutputWebModel.setCategory(storiesData.getCategory());
			fileOutputWebModel.setCategoryRefId(storiesData.getCategoryRefId());

			fileOutputWebModel.setFileId(storiesData.getFileId());
			fileOutputWebModel.setFileName(storiesData.getFileName());
			fileOutputWebModel.setFileType(storiesData.getFileType());
			fileOutputWebModel.setFileSize(storiesData.getFileSize());
			fileOutputWebModel.setFilePath(storiesData.getFilePath());
			fileOutputWebModel.setDescription(storiesData.getDescription());

			fileOutputWebModel.setCreatedBy(storiesData.getCreatedBy());
			fileOutputWebModel.setCreatedOn(storiesData.getCreatedOn());
			fileOutputWebModel.setUpdatedBy(storiesData.getUpdatedBy());
			fileOutputWebModel.setUpdatedOn(storiesData.getUpdatedOn());
