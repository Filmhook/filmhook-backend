...
public class AuditionServiceImpl implements AuditionService {
	...
	public ResponseEntity<?> saveAudition(AuditionWebModel auditionWebModel) {
		...
		return ResponseEntity.status(HttpStatus.OK)
				.body(new Response(1, "Audition details saved successfully", response));
	}
	...
	public ResponseEntity<?> getAuditionByCategory(Integer categoryId) {
		...
		return ResponseEntity.status(HttpStatus.OK)
				.body(new Response(1, "Audition details fetched successfully", response));
	}
	...
	public ResponseEntity<?> auditionAcceptance(AuditionAcceptanceWebModel acceptanceWebModel) {
		...
		return ResponseEntity.status(HttpStatus.OK)
				.body(new Response(1, "Audition acceptance details saved successfully", response));
	}
	...
	@Override
	public ResponseEntity<?> getAddressList(AddressListWebModel addressListWebModel) {
		List<AddressList> addressLists = addressRepository.findAll();
		List<Map<String, Object>> result = addressLists.stream().map(address -> {
			Map<String, Object> map = new HashMap<>();
			map.put("id", address.getAddressListId());
...
//		@Override
//		public ResponseEntity<?> getAuditionByCategoryAndAddress(Integer categoryId, String searchKey) {
		    HashMap<String, Object> response = new HashMap<>();
		    try {
		        ...
		        List<Audition> auditionsByCategory = auditionRepository.findByAuditionCategory(categoryId)
		                .stream()
		                .filter(audition -> !ignoredAuditionIds.contains(audition.getAuditionId()))
		                .collect(Collectors.toList());
		        ...
		        logger.error("get audition by category and address Method Exception...", e);
		        e.printStackTrace();
		        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
		                .body(new Response(-1, "Fail", e.getMessage()));
		    }
